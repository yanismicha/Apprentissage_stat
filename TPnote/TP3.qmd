---
title: "TP3 Kmeans et clustering"
author: 
- Yanis Micha
- Andrew Hardwick
format:
...
Vous trouverez  en haut à droite un bouton permettant de passer le rendu en mode sombre/jour à activer selon votre préférence.  
...
```{r}
Aggregation <- read.delim("data/Aggregation.txt", header=FALSE)
Aggregation<- Aggregation[order(Aggregation$V3), ]
spiral<- read.delim("data/3spiral.txt", header=FALSE,sep = " ")
spiral<- spiral[order(spiral$V3), ]
flame <- read.delim("data/flame.txt",header = FALSE)
...
```{r}
spectra <- read.table("data/spectra.txt")
meta <- read.table("data/meta-data.txt",header = TRUE)
