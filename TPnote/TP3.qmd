...
<a href="https://github.com/yanismicha" class="btn btn-primary">Lien vers le git</a>
...
Les arguments obligatoires dans la fonction <span style="color: #8B0000;">kmeans</span>:
...
Où $a_i$ est la distance moyenne du point avec tout les autres points dans le même cluster
et où $b_i$ est la distance moyenne du point considéré avec tout les points du cluster le plus proche.  
...
<summary style="font-weight: bold; color: #72afd2;">Voir le code</summary>
```{r,eval=FALSE,echo=TRUE}
hclust <- hclust(dist(Aggregation),method = "ward.D")
...
On applique cet algorithme sur notre jeu de données,avec comme clusters $k=7$ et comme ecart-type<br>  $\sigma$=1: 
<details>
<summary style="font-weight: bold; color: #72afd2;">Voir le code</summary>
```{r,eval=FALSE,echo=TRUE}
euclidean_distance_squared <- function(si, sj) {
  sum((si - sj)^2)
}
...
spectral_clust<- function(data,k,sigma){
  ...
  for(i in 1:n){
    for(j in 1:n){
      if(i != j){
        distance_squared <- euclidean_distance_squared(data[i, ], data[j, ])
        A[i, j] <- exp(-distance_squared / (2 * sigma^2))
      }
    }
  }
  diagonal <- rowSums(A)
...
Aggregation$cluster <- factor(spectral_clust(Aggregation[,c(1,2)],7,1))
...
ggplot(Aggregation, aes(x = V1, y = V2, color = cluster)) +
  ...
  ggtitle("Kmeans avec clustering spectral")
```
Pour optimiser le resultat, il faudrait trouver le bon $\sigma$ qui permette une bonne répartition de nos données <br>(mais algorithme coûteux!).  
...
<summary style="font-weight: bold; color: #72afd2;">Voir le code</summary>
```{r,eval=FALSE,echo=TRUE}
hclust <- hclust(dist(flame),method = "ward.D")
...
## 1: réprésentation des observations
```{r}
